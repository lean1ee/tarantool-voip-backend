...
local function count_active_calls(caller)
    ...
    local count = 0
    if dialogs_space.index and dialogs_space.index.by_caller then
        for _, _ in dialogs_space.index.by_caller:pairs(caller) do
            count = count + 1
        end
    else
...
function billing.authorize_call(caller, callee, call_id, node_id)
    ...
    -- Anti-fraud check: max concurrent channels
    ...
    local total_min_required = connect_fee + (rate_per_min / 6.0)
    ...
    -- Calculate maximum allowed duration in seconds based on available balance
    ...
    -- Atomically register the active SIP dialog and reservation
...
function billing.finalize_cdr(call_id, duration_sec, rx_packets, tx_packets, jitter_ms, packet_loss_pct, mos_score, node_id)
    ...
    local start_epoch = extra.start_epoch or (math.floor(fiber.time()) - (duration_sec or 0))
    local end_epoch = math.floor(fiber.time())

    -- Calculate billed amount: per-minute with 1-minute round-up
    local dur = duration_sec or 0
    local billable_minutes = math.ceil(dur / 60.0)
    if dur == 0 then billable_minutes = 0 end
    ...
    log.info("CDR %s finalized: duration=%ds, billed=%.4f, mos=%.2f, remaining_balance=%.2f",
        cdr_id, dur, billed_amount, mos_score or 4.4, new_balance)
...
--- 3. Provisioning Stored Procedures (BSS/OSS API)
function billing.add_subscriber(sub_id, initial_balance, currency, max_calls, tariff_id)
    local subs_space = box.space.subscribers
    if not subs_space then return { ok = false, error = "Space not found" } end
    ...
    return { ok = true, subscriber_id = sub_id, balance = initial_balance }
end

function billing.update_balance(sub_id, delta_amount)
    local subs_space = box.space.subscribers
    if not subs_space then return { ok = false, error = "Space not found" } end

    local sub = subs_space:get(sub_id)
    if not sub then return { ok = false, error = "Subscriber not found" } end

    local now = math.floor(fiber.time())
    local updated = subs_space:update(sub_id, {
        { '+', 2, delta_amount },
        { '=', 7, now }
    })
    return { ok = true, subscriber_id = sub_id, balance = updated[2] }
end

function billing.set_tariff(prefix, description, cost_per_minute, connect_fee)
    local tariffs_space = box.space.tariffs
    if not tariffs_space then return { ok = false, error = "Space not found" } end

    tariffs_space:replace({
        prefix,
        description or prefix,
        cost_per_minute or 0.05,
        connect_fee or 0.0
    })
    return { ok = true, prefix = prefix }
end

--- 4. Live Analytical Dashboard Queries
