...
typedef struct str_core { char *s; int len; } str;
#define str_init(v) { (char *)(v), (int)(sizeof(v) - 1) }

typedef struct param_export_s { const char *name; int type; void *param_pointer; } param_export_t;
typedef struct cmd_export_s { const char *name; void *function; int fixup; int free_fixup; int flags; int extra; } cmd_export_t;
...
typedef struct sip_msg { int id; } sip_msg_t;
typedef struct sr_kemi_s { str mname; str fname; int rtype; void *func; int ptypes[6]; } sr_kemi_t;
static inline int sr_kemi_modules_add(sr_kemi_t *k) { (void)k; return 0; }
#endif
...
static param_export_t params[] = {
	{"server",                 PARAM_STRING, &tnt_server},
	{"host",                   PARAM_STRING, &tnt_host},
	{"port",                   PARAM_INT,    &tnt_port},
	{"user",                   PARAM_STRING, &tnt_user},
	{"pass",                   PARAM_STRING, &tnt_pass},
...
static int mod_init(void)
{
	...
	LM_INFO("Initializing ndb_tarantool connecting to %s:%d (pool=%d, timeout=%dms)...\n",
	        tnt_host, tnt_port, tnt_pool_size, tnt_connect_timeout);

	int rc = tnt_pool_init(&tnt_global_pool, tnt_host, tnt_port, tnt_user, tnt_pass, tnt_pool_size, tnt_connect_timeout);
	if (rc != 0) {
		if (init_without_tarantool) {
			LM_WARN("Failed to connect to Tarantool at bootstrap, but init_without_tarantool is enabled. Continuing...\n");
			return 0;
		}
...
{
	return sr_kemi_tarantool_call(msg, (str_t *)proc, (str_t *)params, (str_t *)res);
}

static int ki_tarantool_eval(sip_msg_t *msg, str *code, str *params, str *res)
{
	return sr_kemi_tarantool_eval(msg, (str_t *)code, (str_t *)params, (str_t *)res);
}

static sr_kemi_t sr_kemi_ndb_tarantool_exports[] = {
	{ str_init("tarantool"), str_init("call"),
		SR_KEMIP_INT, (void *)ki_tarantool_call,
		{ SR_KEMIP_STR, SR_KEMIP_STR, SR_KEMIP_STR,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("tarantool"), str_init("eval"),
		SR_KEMIP_INT, (void *)ki_tarantool_eval,
		{ SR_KEMIP_STR, SR_KEMIP_STR, SR_KEMIP_STR,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
