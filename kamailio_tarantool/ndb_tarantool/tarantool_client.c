 ...
 *
 * Author: lean1ee
 * Module: ndb_tarantool - Binary IProto client for Tarantool 3.x
 *
...
static int check_iproto_status(const char *resp, size_t len, uint64_t want_sync) {
    ...
    for (uint32_t i = 0; i < h_map && p < end; i++) {
        ...
        if (k == IPROTO_REQUEST_TYPE) {
            ...
            if (code != IPROTO_OK) return -1;
        } else if (k == IPROTO_SYNC) {
            uint64_t sync = mp_decode_uint(&p);
            if (sync != want_sync) return -1;
        } else {
...
static int tnt_conn_connect(tnt_conn_t *conn, const char *host, int port, const char *user, const char *pass, int timeout_ms) {
    if (!conn) return -1;
    (void)user;
    (void)pass;
...
int tnt_pool_init(tnt_pool_t *pool, const char *host, int port, const char *user, const char *pass, int pool_size, int timeout_ms) {
    if (!pool || pool_size <= 0) return -1;
...
void tnt_pool_destroy(tnt_pool_t *pool) {
    ...
    if (pool->host) free(pool->host);
    if (pool->user) free(pool->user);
    if (pool->pass) free(pool->pass);
}
...
tnt_conn_t *tnt_pool_get_conn(tnt_pool_t *pool) {
    ...
    if (!conn->connected || conn->fd < 0) {
        tnt_conn_connect(conn, pool->host, pool->port, pool->user, pool->pass, pool->timeout_ms);
    }
    return conn;
}
 ...
 */
int tnt_call_procedure(tnt_conn_t *conn, const char *proc_name, const char *params_json, char *res_buf, size_t res_size) {
    if (!conn || !proc_name) return -1;
    (void)params_json;
    ...
    /* Read response */
    char resp_hdr[5];
    if (tnt_recv_all(conn->fd, resp_hdr, 5) < 0 || (uint8_t)resp_hdr[0] != 0xce) {
        close(conn->fd);
        conn->fd = -1;
        conn->connected = 0;
        return -1;
    ...
    if (res_buf && res_size > 0) {
        snprintf(res_buf, res_size, "{\"status\":\"ok\",\"proc\":\"%s\"}", proc_name);
    }
 ...
 */
int tnt_eval_code(tnt_conn_t *conn, const char *lua_code, const char *params_json, char *res_buf, size_t res_size) {
    if (!conn || !lua_code) return -1;
    (void)params_json;
...
int tnt_save_call_sg(tnt_conn_t *conn, const char *call_id, size_t cid_len,
                     const char *node_id, size_t nid_len,
                     const char *state, size_t state_len,
                     int expires,
                     const char *payload, size_t payload_len) {
    if (!conn || !call_id || cid_len == 0) return -1;
    ...
    char *mp = meta_buf;
    mp = mp_encode_str(mp, node_id ? node_id : "kam_proxy", nid_len ? (uint32_t)nid_len : 9);
    mp = mp_encode_str(mp, state ? state : "active", state_len ? (uint32_t)state_len : 6);
    ...
    size_t body_len = (hdr_len - 5) + cid_len + meta_len + val_hdr_len + payload_len;
    hdr_buf[0] = (char)0xce;
...
int tnt_get_call_buf(tnt_conn_t *conn, const char *call_id, size_t cid_len,
    ...
    if (!conn || !call_id || cid_len == 0 || !dst_buf || dst_len == 0) return -1;
    if (!conn->connected || conn->fd < 0) return -1;
