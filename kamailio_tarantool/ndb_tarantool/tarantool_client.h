 ...
 *
 * Author: lean1ee
 * Module: ndb_tarantool - Connection pool and binary IProto client header for Tarantool 3.x
 *
...
typedef struct tnt_conn {
    ...
    uint64_t sync_id;         /**< Monotonically increasing IProto request sync identifier */
} tnt_conn_t;
 ...
 */
int  tnt_pool_init(tnt_pool_t *pool, const char *host, int port, const char *user, const char *pass, int size, int timeout_ms);
