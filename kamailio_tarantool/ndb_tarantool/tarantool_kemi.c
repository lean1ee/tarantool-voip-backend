...
int sr_kemi_tarantool_call(void *msg, str_t *proc_name, str_t *params_json, str_t *res_dst) {
    (void)msg;
    ...
    char name_buf[128];
    int nlen = proc_name->len < (int)sizeof(name_buf) - 1 ? proc_name->len : (int)sizeof(name_buf) - 1;
    memcpy(name_buf, proc_name->s, nlen);
    name_buf[nlen] = '\0';

    char params_buf[1024];
    if (params_json && params_json->s && params_json->len > 0) {
        int plen = params_json->len < (int)sizeof(params_buf) - 1 ? params_json->len : (int)sizeof(params_buf) - 1;
    ...
    char res_buf[2048];
    int rc = tnt_call_procedure(conn, name_buf, params_buf, res_buf, sizeof(res_buf));
...
int sr_kemi_tarantool_eval(void *msg, str_t *lua_code, str_t *params_json, str_t *res_dst) {
    (void)msg;
    ...
    char res_buf[2048];
    int rc = tnt_eval_code(conn, lua_code->s, params_json ? params_json->s : "[]", res_buf, sizeof(res_buf));
    tnt_pool_put_conn(&tnt_global_pool, conn);
